<?php 
include_once("../includes.php");
if (!isset($_GET["idfuncionario"]))
{
    header("Location:../funcionario/lista_funcionario.php");
}
?>

<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-GLhlTQ8iRABdZLl6O3oVMWSktQOp6b7In1Zl3/Jr59b6EGGoI1aFkw7cmDA6j6gD" crossorigin="anonymous">

<body div class="p-3 mb-2 bg-dark-subtle text-center">



        <div class="modal-content rounded-4 shadow">
      <div class="modal-header border-bottom-0">
    <div class="container">
            <form action="cadastro_cadastros.php" method="POST">
            <input type="hidden" name="idfuncionario" class="form-control" value="
                <?php
                echo $_GET["idfuncionario"];
                ?>">

                <div class="form-group">
                    <label>usuario</label>
                    <input type="text" name="usuario_funcionario" class="form-control" placeholder="Preencha o usuario">                
                </div>
                    <div class="mb-3">                    
                        <label class="form-label">senha</label> 
                        <input type="password" name="senha_funcionario" class="form-control" placeholder="Preencha o seu senha">      
                        <button type="submit" name="salvarcadastro" class="btn btn-primary">Registrar</button>             
                    </div>
    </form>
</body>
</html>

